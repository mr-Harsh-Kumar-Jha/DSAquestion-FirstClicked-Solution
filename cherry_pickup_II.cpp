...
// this is a recursive approach and time complexity is exponential O(3^n * 3^n) and space complexity is O(r*c) which is auxilliary space.

#include<bits/stdc++.h>
using namespace std;

int cherryPick(vector<vector<int>>&vec,int i2,int j1,int j2 ){
      ...
      for(int i=-1;i<=1;i++){
         for(int j=-1;j<=1;j++){
            ...
            value+=cherryPick(vec,i2+1,j1+i,j2+j);
            maxi=max(maxi,value);
...
int main(){
   ...
   cout<<cherryPick(vec,0,0,vec[0].size()-1)<<endl;
}