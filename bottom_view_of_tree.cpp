...
vector<int> bottomViewTraversal(node*root){
   ...
   pair.push({root,0});
   while(!pair.empty()){
      auto temp = pair.front();
      pair.pop();
      int x = temp.second;
      map[x].push_back(temp.first->data);
      if(temp.first->left) pair.push({temp.first->left, x-1});
      if(temp.first->right) pair.push({temp.first->right, x+1});
   }
   vector<int>ans;
   for(auto it:map){
...
int main(){
   ...
   seventh1->data=1;
   eighth1->data=0;
  vector<int>ans= bottomViewTraversal(Head1);
